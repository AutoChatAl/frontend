...
export default function SupportChatWidget() {
  const { isOpen, loading, sending, conversation, messages, error, closedConversationMessage, workspaceConfigured, setIsOpen, startNewConversation, sendMessage } = useSupportChat();
  ...
  return (<>
    ...
    {isOpen && (<div className="fixed bottom-0 right-6 z-40 w-[min(92vw,420px)] rounded-t-3xl rounded-b-none border border-slate-200 bg-white shadow-2xl shadow-slate-900/15 dark:border-slate-800 dark:bg-slate-950 dark:shadow-black/40">
      <div className="flex items-center justify-between rounded-t-3xl bg-indigo-600 px-5 py-4 text-white dark:bg-slate-900">
        ...
        <button type="button" onClick={() => setIsOpen(false)} className="rounded-full p-1 text-slate-300 transition hover:bg-white/10 hover:text-white">
          <X size={18}/>
        </button>
      </div>
      ...
      <div className="flex h-140 flex-col">
        <div ref={messagesContainerRef} className="flex-1 space-y-3 overflow-y-auto bg-slate-50 px-4 py-4 dark:bg-slate-900/70">
          {!workspaceConfigured && (<div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-700 dark:border-amber-900 dark:bg-amber-950/40 dark:text-amber-200">
                  Configure `NEXT_PUBLIC_SUPPORT_WORKSPACE_ID` para habilitar o suporte.
          </div>)}

          {closedConversationMessage ? (<div className="flex min-h-full items-center justify-center py-6">
            <div className="w-full rounded-3xl border border-slate-200 bg-white px-5 py-6 text-center shadow-sm dark:border-slate-800 dark:bg-slate-950">
              ...
              <button type="button" onClick={startNewConversation} className="mt-4 inline-flex items-center justify-center rounded-xl bg-indigo-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-indigo-700">
                      Iniciar nova conversa
              </button>
            </div>
          </div>) : messages.length === 0 && !loading ? (<div className="rounded-2xl border border-dashed border-slate-300 bg-white px-4 py-6 text-center text-sm text-slate-500 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-300">
                  Envie a primeira mensagem para iniciar o atendimento.
          </div>) : null}

          {!closedConversationMessage && messages.map((message: {
                id: string;
                senderType: string;
                body?: string | null;
            ...
            return (<div key={message.id}>
              {showDateSeparator && (<div className="mb-2 flex justify-center">
              ...
              <div className={`flex ${isVisitor ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[82%] rounded-2xl border px-4 py-3 text-sm shadow-sm ${isVisitor
                  ...
                  {message.imageBase64 && (<img src={`data:${message.imageMimeType || 'image/png'};base64,${message.imageBase64}`} alt="Imagem enviada no suporte" className="mt-2 max-h-56 w-full rounded-xl object-cover"/>)}
        ...
        <div className="border-t border-slate-200 bg-white px-4 py-4 dark:border-slate-800 dark:bg-slate-950">
          ...
          {!closedConversationMessage && <div className="mt-3 flex items-center justify-between gap-3">
            <div className="flex items-center gap-2">
              ...
              <button type="button" onClick={() => fileRef.current?.click()} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 px-3 py-2 text-sm font-medium text-slate-600 transition hover:bg-slate-50 dark:border-slate-800 dark:text-slate-300 dark:hover:bg-slate-900">
                <ImagePlus size={16}/>
                    Imagem
              </button>
            </div>

            <button type="button" onClick={handleSubmit} disabled={sending || (!draft.trim() && !imageFile)} className="inline-flex items-center gap-2 rounded-xl bg-indigo-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-indigo-700 disabled:cursor-not-allowed disabled:opacity-60">
              <Send size={16}/>
