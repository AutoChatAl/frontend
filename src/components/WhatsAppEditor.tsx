...
const FORMAT_MAP: Record<FormatType, {
    prefix: string;
    suffix: string;
}> = {
  bold: { prefix: '*', suffix: '*' },
...
export default function WhatsAppEditor({ value, onChange, placeholder = 'Digite a mensagem...', rows = 5, error, maxLength }: WhatsAppEditorProps) {
  ...
  return (<div className="space-y-1.5">
    <div className="border rounded-xl overflow-hidden border-slate-200 dark:border-slate-700 focus-within:ring-2 focus-within:ring-indigo-500/20 focus-within:border-indigo-400 transition-colors">
      <div className="flex items-center gap-1 px-3 py-2 bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-700">
        {buttons.map(({ format, icon: Icon, label, shortcut }) => (<button key={format} type="button" onClick={() => applyFormat(format)} title={`${label} (${shortcut})`} className="p-1.5 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors">
          <Icon size={16}/>
        </button>))}
      ...
      <textarea ref={textareaRef} value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeholder} rows={rows} maxLength={maxLength} className="w-full px-4 py-2.5 text-sm bg-white dark:bg-slate-900 text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none resize-none"/>
    </div>
