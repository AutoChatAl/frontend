...
export default function PixPaymentModal({ isOpen, onClose, planSlug, planName }: PixPaymentModalProps) {
  ...
  return (<Modal isOpen={isOpen} onClose={handleClose} title="Pagar com PIX" size="sm">
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between p-3 bg-indigo-50 dark:bg-indigo-900/20 rounded-lg">
        ...
        {pixData && (<span className="text-lg font-bold text-indigo-600 dark:text-indigo-400">
          {formatBRL(pixData.amount)}<span className="text-xs font-normal text-slate-500">/mês</span>
        </span>)}
      </div>

      {loading && (<div className="flex flex-col items-center justify-center py-10 gap-3">
        <div className="w-8 h-8 border-2 border-indigo-500 border-t-transparent rounded-full animate-spin"/>
        <p className="text-sm text-slate-500 dark:text-slate-400">Gerando QR Code...</p>
      </div>)}

      {error && !loading && (<div className="text-center py-6 space-y-3">
      ...
      {pixData && !loading && (<>
        <div className="flex flex-col items-center gap-3">
          {pixData.qrCodeImageUrl ? (<div className="p-3 bg-white rounded-xl border border-slate-200 dark:border-slate-600 shadow-sm">

            <img src={pixData.qrCodeImageUrl} alt="QR Code PIX" className="w-48 h-48 object-contain"/>
          </div>) : (<div className="p-4 bg-slate-100 dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 flex flex-col items-center gap-2">
            <QrCode size={80} className="text-slate-400"/>
            <p className="text-xs text-slate-500">Use o código copia e cola abaixo</p>
          </div>)}

          {timeLeft && (<div className="flex items-center gap-1.5 text-xs text-amber-600 dark:text-amber-400">
            <Clock size={12}/>
            <span>Expira em: <strong>{timeLeft}</strong></span>
          </div>)}
        </div>

        {pixData.qrCodeString && (<div>
          <p className="text-xs font-medium text-slate-600 dark:text-slate-400 mb-1.5">Código PIX copia e cola:</p>
          <div className="flex items-center gap-2">
            <input readOnly value={pixData.qrCodeString} className="flex-1 text-xs bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-600 rounded-lg px-3 py-2 text-slate-700 dark:text-slate-300 truncate font-mono"/>
            <button onClick={handleCopy} className="p-2 rounded-lg border border-slate-200 dark:border-slate-600 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors text-slate-500 dark:text-slate-400 shrink-0" title="Copiar código">
