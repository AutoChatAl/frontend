export default function Badge({ type, text, icon: Icon }: { type: string; text: string; icon?: React.ComponentType<{ size: number }> }) {
  const styles = {
  ...
  return (
    <span className={`px-2.5 py-1 rounded-md text-xs font-semibold border flex items-center gap-1 w-fit transition-colors duration-200 ${styles[type as keyof typeof styles] || styles.neutral}`}>
      {Icon && <Icon size={12} />}
      {text}
    </span>
  );
