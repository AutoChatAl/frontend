...
export default function Select({ label, error, hint, leftIcon, options, wrapperClassName = '', className: _, id, ...props }: SelectProps) {
  const selectId = id ?? (label ? label.toLowerCase().replace(/\s+/g, '-') : undefined);
  return (<div className={`space-y-1.5 ${wrapperClassName}`}>
    {label && (<label htmlFor={selectId} className="block text-sm font-medium text-slate-700 dark:text-slate-300">
      {label}
    </label>)}
    <div className="relative">
      ...
      <select id={selectId} {...props} className={[
