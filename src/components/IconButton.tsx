...
export default function IconButton({ icon, onClick, title, variant = 'default', size = 'sm', disabled = false, className = '' }: IconButtonProps) {
  const sizeStyles = {
    sm: 'p-1.5',
    md: 'p-2',
  };
  ...
  return (<button type="button" onClick={onClick} title={title} disabled={disabled} className={`${sizeStyles[size]} rounded-lg transition-colors ${variantStyles[variant]} ${disabled ? 'opacity-40 cursor-not-allowed' : ''} ${className}`}>
    {icon}
  </button>);
}
