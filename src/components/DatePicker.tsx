...
export default function DatePicker({ value, onChange, min, placeholder = 'Selecione uma data' }: DatePickerProps) {
  ...
  return (<div ref={ref} className="relative w-full min-w-0">
    <button type="button" onClick={() => setOpen((v) => !v)} className={`w-full min-w-0 overflow-hidden flex items-center gap-2 px-3 py-2.5 border rounded-xl text-sm transition-all bg-white dark:bg-slate-900 ${open
      ...
      <span className={`flex-1 min-w-0 text-left text-[14px] whitespace-nowrap tabular-nums ${value ? 'text-slate-900 dark:text-white font-normal' : 'text-slate-400 dark:text-slate-500'}`}>
    ...
    {open && (<div className="absolute top-full left-0 mt-2 z-50 w-72 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-xl shadow-slate-900/10 dark:shadow-black/30 p-4 animate-in fade-in zoom-in-95 slide-in-from-top-2 duration-150">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <button type="button" onClick={nextMonth} className="w-8 h-8 rounded-lg flex items-center justify-center text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
          <ChevronRight size={16}/>
        </button>
      </div>

      <div className="grid grid-cols-7 mb-1.5">
        {DAYS_PT.map((d) => (<div key={d} className="text-center text-[10px] font-semibold text-slate-400 dark:text-slate-500 tracking-wide py-1">
          {d}
        </div>))}
      </div>
      ...
      <div className="grid grid-cols-7 gap-0.5">
        {cells.map((date, i) => {
          ...
          return (<button key={i} type="button" disabled={isDisabled} onClick={() => handleSelect(date)} className={`aspect-square flex items-center justify-center rounded-lg text-xs transition-all ${isSelected
      ...
      <div className="mt-3 pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          {value && (<button type="button" onClick={() => onChange('')} className="text-xs font-semibold text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 transition-colors">
                  Limpar
          </button>)}
          <button type="button" onClick={goToday} className="text-xs font-semibold text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300 transition-colors">
                Hoje
          </button>
        </div>
      </div>
