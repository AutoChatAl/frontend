...
function isValidCPF(cpf: string): boolean {
  ...
  if (rem === 10)
    rem = 0;
  if (rem !== parseInt(digits.charAt(9), 10))
    return false;
  sum = 0;
  for (let i = 0; i < 10; i++)
...
function PersonalForm({ onNext, plan, initialName = '', initialCpf = '', initialPhone = '' }: PersonalFormProps) {
  ...
  return (<form onSubmit={handleSubmit} className="space-y-4">
    ...
    <div>
      ...
      <input type="text" value={name} onChange={(e) => { setName(e.target.value); setErrors((prev) => ({ ...prev, name: '' })); }} placeholder="Seu nome completo" className="w-full border border-slate-300 dark:border-slate-600 rounded-lg px-3 py-2.5 text-sm bg-white dark:bg-slate-800 text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"/>
    ...
    <div>
      ...
      <input type="text" value={cpf} onChange={(e) => { setCpf(formatCPF(e.target.value)); setErrors((prev) => ({ ...prev, cpf: '' })); }} placeholder="000.000.000-00" className="w-full border border-slate-300 dark:border-slate-600 rounded-lg px-3 py-2.5 text-sm bg-white dark:bg-slate-800 text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"/>
    ...
    <div>
      ...
      <input type="text" value={phone} onChange={(e) => { setPhone(formatPhone(e.target.value)); setErrors((prev) => ({ ...prev, phone: '' })); }} placeholder="(00) 00000-0000" className="w-full border border-slate-300 dark:border-slate-600 rounded-lg px-3 py-2.5 text-sm bg-white dark:bg-slate-800 text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"/>
...
function CardForm({ plan, personal, onSuccess, onBack }: CardFormProps) {
  ...
  return (<form onSubmit={handleSubmit} className="space-y-4">
    <button type="button" onClick={onBack} className="flex items-center gap-1 text-xs text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 mb-1">
...
export default function PlanCheckoutModal({ isOpen, onClose, plan, onSuccess, initialPersonal }: PlanCheckoutModalProps) {
  ...
  return (<Modal isOpen={isOpen} onClose={handleClose} title={stepTitle[step]} size="sm">
    {step === 'success' ? (<div className="flex flex-col items-center justify-center py-8 gap-3">
