...
export default function OAuthCallbackError({ error, reset }: {
    error: Error & {
        digest?: string;
    };
    reset: () => void;
  ...
  return (<div className="min-h-screen flex items-center justify-center bg-slate-50 p-4">
    <div className="bg-white rounded-2xl shadow-xl p-8 max-w-md w-full text-center space-y-4">
      <h1 className="text-xl font-bold text-red-600">Erro ao carregar a página</h1>
      ...
      <button type="button" onClick={reset} className="px-4 py-2 bg-indigo-600 text-white rounded-lg text-sm font-medium hover:bg-indigo-700">
          Tentar novamente
      </button>
    </div>
  </div>);
