...
function ChannelCard({ icon: Icon, title, subtitle, bgColor, iconColor, highlight, delay }: ChannelCardProps) {
  ...
  return (<div className={`${baseClasses} ${highlightClasses} ${animationClasses}`}>
    <div className={`w-16 h-16 ${highlight ? 'bg-white/20 backdrop-blur-sm' : bgColor} rounded-full flex items-center justify-center mb-4 ${highlight ? 'text-white' : iconColor}`}>
      <Icon size={32}/>
    </div>
...
export default function ChannelsSection() {
  ...
  return (<section id="canais" className="py-24 bg-white dark:bg-slate-900 relative overflow-hidden">
    <div className="max-w-7xl mx-auto px-4 flex flex-col md:flex-row items-center gap-16">
      <div className="w-full md:w-1/2 relative">
        ...
        <div className="relative z-10 grid grid-cols-2 gap-4">
          ...
          <div className="space-y-4">
            <ChannelCard icon={Smartphone} title="Instagram" subtitle="Directs & Stories" bgColor="bg-fuchsia-100" iconColor="text-fuchsia-600"/>
            <ChannelCard icon={Bot} title="IA Central" subtitle="Processamento" bgColor="" iconColor="" highlight/>
          </div>
        </div>
      </div>
      ...
      <div className="w-full md:w-1/2">
        ...
        <ul className="space-y-4">
          {benefits.map((item, i) => (<li key={i} className="flex items-center gap-3 text-slate-700 dark:text-slate-300 font-medium">
