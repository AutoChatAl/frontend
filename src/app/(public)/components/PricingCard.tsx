...
export default function PricingCard({ name, price, period, description, features, highlighted = false, ctaText = 'Começar agora' }: PricingCardProps) {
  return (<div className={`relative p-8 rounded-2xl border-2 transition-all hover:scale-105 ${highlighted
    ...
    {highlighted && (<div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-amber-400 text-slate-900 px-4 py-1 rounded-full text-xs font-bold">
          MAIS POPULAR
    </div>)}
    ...
    <div className={highlighted ? 'text-white' : 'text-slate-800 dark:text-white'}>
      ...
      <button className={`w-full py-3 px-6 rounded-lg font-semibold mb-8 transition-all ${highlighted
