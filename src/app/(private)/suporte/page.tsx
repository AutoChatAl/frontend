...
export default function SupportPage() {
  ...
  useEffect(() => {
    ...
    const handleMessageCreated = (event: Event) => {
      ...
      setMessages((prev) => (prev.some((message) => message.id === nextMessage.id)
        ? prev
        : [...prev, nextMessage]));
      if (nextConversation) {
      ...
      if (suppressNextMessageCreatedSyncRef.current
  ...
  return (<div className="space-y-6">
    <ConfirmDeleteModal isOpen={isDeleteModalOpen} onClose={() => {
      if (!deleting)
        setIsDeleteModalOpen(false);
    }} onConfirm={handleDeleteConversation} title="Deletar conversa" message="Tem certeza que deseja deletar esta conversa de suporte? Todas as mensagens vinculadas também serão removidas permanentemente." confirmLabel="Deletar conversa" loading={deleting}/>
    ...
    <div className="grid gap-4 xl:grid-cols-[360px_minmax(0,1fr)]">
      <section className="flex h-[70vh] min-h-0 max-h-[70vh] flex-col rounded-3xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-950">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {filters.map((filter) => (<button key={filter.value} type="button" onClick={() => setStatusFilter(filter.value)} className={`rounded-full px-3 py-1.5 text-xs font-semibold transition ${statusFilter === filter.value ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200 dark:bg-slate-900 dark:text-slate-300 dark:hover:bg-slate-800'}`}>
            {filter.label}
          </button>))}
        </div>
        ...
        <div className="mt-4 min-h-0 flex-1 space-y-3 overflow-y-auto pr-1">
          {loading ? (<div className="flex items-center justify-center py-10 text-sm text-slate-500"><Loader2 size={18} className="mr-2 animate-spin"/>Carregando conversas...</div>) : conversations.length === 0 ? (<div className="rounded-2xl border border-dashed border-slate-200 px-4 py-8 text-center text-sm text-slate-500 dark:border-slate-800 dark:text-slate-400">
            ...
            <div className="mt-2 flex items-center gap-2">
              ...
              {conversation.unreadByAdminCount > 0 && (<span className="shrink-0 rounded-full bg-amber-500 px-2 py-0.5 text-[10px] font-bold text-white">
      ...
      <section className="flex h-[70vh] min-h-0 max-h-[70vh] flex-col rounded-3xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-950">
        {!selectedConversation ? (<div className="flex flex-1 flex-col items-center justify-center gap-3 px-6 text-center text-slate-500 dark:text-slate-400">
          ...
          <div className="flex flex-col gap-4 border-b border-slate-200 px-6 py-5 dark:border-slate-800 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <button type="button" onClick={handleCloseConversation} disabled={closing || selectedConversation.status === 'CLOSED'} className="inline-flex items-center gap-2 rounded-xl border border-rose-200 px-3 py-2 text-sm font-semibold text-rose-600 transition hover:bg-rose-50 disabled:cursor-not-allowed disabled:opacity-60 dark:border-rose-900 dark:hover:bg-rose-950/30">
                <XCircle size={16}/>
                {closing ? 'Encerrando...' : 'Encerrar atendimento'}
              </button>
              <IconButton icon={<Trash2 size={16}/>} onClick={() => setIsDeleteModalOpen(true)} title="Deletar conversa" variant="danger" size="md" disabled={deleting} className="border border-red-200 dark:border-red-900"/>
            </div>
          </div>
          ...
          <div ref={threadRef} className="min-h-0 flex-1 space-y-3 overflow-y-auto bg-slate-50 px-6 py-5 dark:bg-slate-900/50">
            {threadLoading ? (<div className="flex items-center justify-center py-10 text-sm text-slate-500"><Loader2 size={18} className="mr-2 animate-spin"/>Carregando conversa...</div>) : (<>
              ...
              {selectedConversation.closedAt && (<div className="flex justify-center">
          ...
          <div className="border-t border-slate-200 px-6 py-5 dark:border-slate-800">
            {imageFile && (<div className="mb-3 flex items-center justify-between rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-600 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-300">
              <span className="truncate">{imageFile.name}</span>
              <button type="button" onClick={() => { setImageFile(null); if (fileRef.current)
                fileRef.current.value = ''; }} className="text-rose-500">
                      Remover
              </button>
            </div>)}
            ...
            <div className="flex flex-col gap-3 lg:flex-row lg:items-end">
              ...
              <div className="flex items-center gap-2 lg:flex-col">
                ...
                <button type="button" onClick={() => fileRef.current?.click()} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 px-4 py-3 text-sm font-medium text-slate-600 transition hover:bg-slate-50 dark:border-slate-800 dark:text-slate-300 dark:hover:bg-slate-900">
                  <ImagePlus size={16}/>
                      Imagem
                </button>
                <button type="button" onClick={handleSend} disabled={sending || (!draft.trim() && !imageFile) || selectedConversation.status === 'CLOSED'} className="inline-flex items-center gap-2 rounded-xl bg-indigo-600 px-4 py-3 text-sm font-semibold text-white transition hover:bg-indigo-700 disabled:cursor-not-allowed disabled:opacity-60">
