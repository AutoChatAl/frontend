...
export default function SettingsNav({ activeTab, onTabChange }: SettingsNavProps) {
  ...
  const visibleItems = ALL_NAV_ITEMS.filter((item) => {
    if (item.ownerOnly && role === 'collaborator')
      return false;
    return true;
  });
  return (<div className="grid gap-1.5 sm:gap-1 bg-slate-100 dark:bg-slate-800 p-1 sm:p-1 rounded-lg" style={{ gridTemplateColumns: `repeat(${Math.min(visibleItems.length, 5)}, minmax(0, 1fr))` }}>
    {visibleItems.map((item) => {
      const isActive = activeTab === item.id;
      return (<button key={item.id} onClick={() => onTabChange(item.id)} className={`flex flex-col sm:flex-row items-center justify-center gap-1 sm:gap-1.5 px-2 sm:px-3 py-2 sm:py-2 text-[10px] sm:text-xs font-medium rounded-md transition-all ${isActive
