'use client';
import { Check, Copy, ShieldCheck, ShieldOff, Shield, Smartphone, Download, QrCode, KeyRound } from 'lucide-react';
...
const APPS = [
  {
    ...
    vendor: 'Google',
    icon: (<svg viewBox="0 0 48 48" className="w-8 h-8">
...
function DisableView({ onDisable, onClose }: {
  ...
  return (<div className="space-y-6">
    ...
    <div className="space-y-2">
      ...
      <Input label="Código do autenticador" placeholder="000000" value={code} onChange={(e) => { setCode(e.target.value.replace(/\D/g, '').slice(0, 6)); setError(''); }} maxLength={6} autoComplete="one-time-code" error={error}/>
    </div>
...
function SetupView({ setupData, onVerify, onClose }: {
    setupData: {
        secret: string;
        qrCode: string;
    };
  ...
  return (<div className="space-y-6">
    <div>
      ...
      <div className="flex gap-2 p-1 bg-slate-100 dark:bg-slate-800 rounded-xl">
        {APPS.map((a, i) => (<button key={a.id} type="button" onClick={() => setActiveApp(i)} className={`flex-1 flex flex-col items-center gap-1.5 py-2.5 px-2 rounded-lg text-xs font-medium transition-all select-none ${activeApp === i
    ...
    {step === 'scan' && (<div className="space-y-5">
      <ol className="space-y-2.5">
        {app.steps.map((s, i) => (<li key={i} className="flex items-start gap-3 text-sm text-slate-600 dark:text-slate-400">
      ...
      <div className="flex gap-2">
        <a href={app.ios} target="_blank" rel="noopener noreferrer" className="flex-1 flex items-center justify-center gap-1.5 py-2 px-3 text-xs font-medium bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-lg transition-colors">
          <Download size={12}/> App Store
        </a>
        <a href={app.android} target="_blank" rel="noopener noreferrer" className="flex-1 flex items-center justify-center gap-1.5 py-2 px-3 text-xs font-medium bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-lg transition-colors">
        ...
        <div className="flex-1 min-w-0 space-y-3 w-full">
          <div>
            ...
            <div className="flex items-center gap-2">
              ...
              <button type="button" onClick={handleCopy} title="Copiar chave" className="shrink-0 p-2 rounded-lg text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors">
                {copied ? <Check size={15} className="text-emerald-500"/> : <Copy size={15}/>}
              </button>
            </div>
          </div>

          <button type="button" onClick={() => setStep('verify')} className="w-full flex items-center justify-center gap-2 py-2.5 px-4 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium rounded-xl transition-colors">
            <KeyRound size={15}/>
    ...
    {step === 'verify' && (<div className="space-y-5">
      ...
      <Input label="Código de verificação" placeholder="000000" value={code} onChange={(e) => { setCode(e.target.value.replace(/\D/g, '').slice(0, 6)); setError(''); }} maxLength={6} autoComplete="one-time-code" error={error}/>

      <div className="flex gap-3 justify-end pt-2 border-t border-slate-100 dark:border-slate-800">
        <Button variant="ghost" onClick={onClose}>Cancelar</Button>
        <Button onClick={handleVerify} loading={loading} loadingText="Verificando..." icon={<ShieldCheck size={16}/>}>
              Verificar e Ativar
        </Button>
      </div>
    </div>)}
...
function IdleView({ onSetup, loading, onClose }: {
    onSetup: () => void;
    loading: boolean;
    onClose: () => void;
}) {
  return (<div className="space-y-6">
    ...
    <ul className="space-y-2.5">
      ...
      ].map((b) => (<li key={b} className="flex items-center gap-2.5 text-sm text-slate-600 dark:text-slate-400">
...
export default function TwoFactorModal({ isOpen, enabled, onClose, onSuccess }: TwoFactorModalProps) {
  ...
  return (<Modal isOpen={isOpen} onClose={handleClose} title={title} size="md">
    {enabled ? (<DisableView onDisable={handleDisable} onClose={handleClose}/>) : setupData ? (<SetupView setupData={setupData} onVerify={handleVerify} onClose={handleClose}/>) : (<IdleView onSetup={handleSetup} loading={setupLoading} onClose={handleClose}/>)}
  </Modal>);
}
