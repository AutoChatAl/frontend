...
export default function CollaboratorsPieChart({ slices }: Props) {
  ...
  if (total === 0) {
    return (<div>
      ...
      <div className="mt-3 max-h-[72px] overflow-y-auto">
        <div className="flex flex-wrap justify-center gap-x-3 gap-y-1.5">
          {slices.map((sl, i) => (<div key={i} className="flex items-center gap-1.5 rounded px-1.5 py-0.5">
    ...
    return (<div>
      <div className="flex justify-center">
        <div className="relative" style={{ width: '60%', maxWidth: 220, minWidth: 120, aspectRatio: '1/1' }} onMouseLeave={() => setHoveredIdx(null)}>
          <svg viewBox={`0 0 ${VB} ${VB}`} className="w-full h-full">
            {arcs.map((a) => {
              const isHover = hoveredIdx === a.idx;
              return (<path key={a.idx} d={arcPath(a.startAngle, a.endAngle, isHover ? R_HOVER : R)} fill={a.slice.color} opacity={hoveredIdx !== null && !isHover ? 0.5 : 1} stroke="white" strokeWidth={1.5} className="cursor-pointer transition-opacity" onMouseEnter={() => setHoveredIdx(a.idx)}/>);
            })}
          </svg>

          {hovered && (<div className="absolute inset-0 flex items-center justify-center pointer-events-none">
      ...
      <div className="mt-3 max-h-[72px] overflow-y-auto">
        <div className="flex flex-wrap justify-center gap-x-3 gap-y-1.5">
          {slices.map((sl, i) => (<div key={i} className="flex items-center gap-1.5 cursor-pointer rounded px-1.5 py-0.5 hover:bg-slate-100 dark:hover:bg-slate-700/50 transition-colors" onMouseEnter={() => {
            const visIdx = visibleSlices.findIndex((v) => v.name === sl.name);
            if (visIdx >= 0)
              setHoveredIdx(visIdx);
