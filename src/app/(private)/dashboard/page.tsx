'use client';
import { Send, MessageSquare, CheckCheck, Users, Wifi, Megaphone, AlertCircle, DollarSign, CalendarCheck, Package } from 'lucide-react';
...
export default function DashboardPage() {
  ...
  if (error || !metrics) {
    return (<div className="flex items-center justify-center h-64">
      <div className="text-center">
        ...
        <button onClick={loadMetrics} className="mt-3 text-sm text-indigo-600 hover:text-indigo-700 font-medium">
            Tentar novamente
        </button>
      </div>
    </div>);
  ...
  return (<div className="space-y-6 w-full">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
      ...
      {isOwner && collabMetrics && collabMetrics.length > 0 ? (<div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-4 shadow-sm flex flex-col">
        <div className="mb-2 shrink-0">
        ...
        <div className="flex-1 flex flex-col justify-center">
          <CollaboratorsPieChart slices={collabMetrics.map((c, i) => ({
            name: c.name || c.email,
            value: c.messagesSent,
            color: PIE_PALETTE[i % PIE_PALETTE.length] ?? '#6366f1',
          }))}/>
        </div>
      </div>) : (<div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-4 shadow-sm flex flex-col">
        <div className="mb-2 shrink-0">
    ...
    {metrics.messagesFailed > 0 && (<div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl p-4 flex items-start gap-3">
    ...
    {billing && (<>
      <div className="pt-2">
        <h2 className="text-2xl font-bold text-slate-800 dark:text-white">Faturamento</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <MetricCard title="Faturamento Total" value={formatCurrency(billing.totalRevenueCents)} icon={DollarSign} color="text-green-600 dark:text-green-400" bgColor="bg-green-50 dark:bg-green-900/30"/>
      ...
      {billing.revenueByProduct.length > 0 && (<div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-5 shadow-sm">
