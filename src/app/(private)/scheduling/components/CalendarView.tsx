...
export default function CalendarView({ appointments, businessHours, contacts: _contacts, products: _products, currentWeekStart, onWeekChange, onCreateAppointment, onEditAppointment, onUpdateStatus: _onUpdateStatus }: CalendarViewProps) {
  ...
  return (<div className="space-y-4">
    ...
    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-3 sm:p-4">
      <div className="flex items-center gap-2 w-full sm:w-auto">
        ...
        <button onClick={goToToday} className="px-3 py-1.5 text-xs font-medium bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400 rounded-lg hover:bg-indigo-100 dark:hover:bg-indigo-900/30 transition-colors">
            Hoje
        </button>
        <button onClick={() => navigateWeek(1)} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg transition-colors">
      ...
      <div className="flex space-x-1 bg-slate-100 dark:bg-slate-700 p-1 rounded-lg w-full sm:w-auto">
        <button onClick={() => setViewMode('week')} className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-md transition-all flex-1 sm:flex-initial justify-center ${viewMode === 'week' ? 'bg-white dark:bg-slate-600 text-indigo-700 dark:text-indigo-300 shadow-sm' : 'text-slate-500 dark:text-slate-400'}`}>
          <CalendarDays size={14}/>
            Semana
        </button>
        <button onClick={() => setViewMode('month')} className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-md transition-all flex-1 sm:flex-initial justify-center ${viewMode === 'month' ? 'bg-white dark:bg-slate-600 text-indigo-700 dark:text-indigo-300 shadow-sm' : 'text-slate-500 dark:text-slate-400'}`}>
          <LayoutGrid size={14}/>
            Mês
        </button>
      </div>
      ...
      <div className="sm:hidden bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
        ...
        <div className="flex border-b border-slate-200 dark:border-slate-700">
          {weekDays.map((day, i) => {
            ...
            return (<button key={i} onClick={() => setMobileSelectedDay(day)} className={`flex-1 py-2 flex flex-col items-center gap-0.5 transition-colors ${isSelected ? 'bg-indigo-50 dark:bg-indigo-900/10' : ''}`}>
              <span className="text-[9px] font-medium text-slate-400 dark:text-slate-500 uppercase">
                {DAY_SHORT[day.getDay()]}
              </span>
              <span className={`w-6 h-6 flex items-center justify-center rounded-full text-xs font-bold ${isToday(day)
        ...
        {monthDays.map((day, idx) => {
          ...
          return (<div key={idx} onClick={() => onCreateAppointment(formatDateStr(day))} className={`min-h-17.5 sm:min-h-25 p-1 sm:p-2 border-r border-b border-slate-100 dark:border-slate-700 cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors ${idx % 7 === 6 ? 'border-r-0' : ''} ${!isCurrMonth ? 'opacity-40' : ''} ${isToday(day) ? 'bg-indigo-50/50 dark:bg-indigo-900/10' : ''}`}>
            ...
            <div className="space-y-0.5">
              ...
              {dayAppts.length > 3 && (<div className="text-[9px] sm:text-[11px] text-slate-400 dark:text-slate-500 px-1">
