...
export default function AIProductsInput({ products, onAddProduct, onUpdateProduct, onDeleteProduct }: AIProductsInputProps) {
  ...
  return (<div className="space-y-3">
    ...
    <div className="flex flex-wrap items-center gap-2 w-full px-3 py-2.5 border border-slate-200 dark:border-slate-700 rounded-xl bg-white dark:bg-slate-900 focus-within:ring-2 focus-within:ring-indigo-500/20 focus-within:border-indigo-500 transition-colors cursor-text min-h-10.5" onClick={() => inputRef.current?.focus()}>
      {products.map((p) => (<span key={p.id} className="inline-flex items-center gap-1 px-2.5 py-1 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 text-xs font-medium rounded-full border border-indigo-100 dark:border-indigo-800">
        {p.name}
        <button type="button" onClick={(e) => { e.stopPropagation(); onDeleteProduct(p.id); }} className="ml-0.5 hover:text-red-500 dark:hover:text-red-400 transition-colors">
          <X size={12}/>
        </button>
      </span>))}
      <input ref={inputRef} type="text" value={inputValue} onChange={handleChange} onKeyDown={handleKeyDown} placeholder={products.length === 0 ? 'Digite e pressione Enter ou vírgula para adicionar...' : 'Adicionar...'} className="flex-1 min-w-30 bg-transparent border-none outline-none text-sm text-slate-700 dark:text-slate-200 placeholder:text-slate-300 dark:placeholder:text-slate-500"/>
    </div>
      ...
      <div className="hidden sm:block overflow-x-auto rounded-xl border border-slate-200 dark:border-slate-700">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
            {products.map((product) => (<ProductRow key={product.id} product={product} onUpdate={onUpdateProduct} onDelete={onDeleteProduct} formatCents={formatCents}/>))}
          </tbody>
        </table>
      </div>

      <div className="sm:hidden space-y-3">
        {products.map((product) => (<ProductCard key={product.id} product={product} onUpdate={onUpdateProduct} onDelete={onDeleteProduct} formatCents={formatCents}/>))}
      </div>
    </>)}
  </div>);
}
function ProductRow({ product, onUpdate, onDelete, formatCents }: {
    product: Product;
  ...
  return (<tr className="bg-white dark:bg-slate-800/50 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
    ...
    <td className="px-4 py-2.5">
      <input type="text" value={link} onChange={(e) => setLink(e.target.value)} onBlur={handleLinkBlur} className={cellInput} placeholder="https://..."/>
    </td>
...
function ProductCard({ product, onUpdate, onDelete, formatCents }: {
    product: Product;
    onUpdate: (id: string, data: {
        name?: string;
        priceCents?: number;
