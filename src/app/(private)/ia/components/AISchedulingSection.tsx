'use client';
import { CalendarDays, MessageSquare, CalendarCheck, Lock } from 'lucide-react';
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between gap-3">
              <h4 className="text-sm font-semibold text-slate-700 dark:text-slate-200 flex items-center gap-1.5">
                ...
                {!schedulingQueryAllowed && <Lock size={12} className="text-amber-500"/>}
              </h4>
              <button
                onClick={() => schedulingQueryAllowed && onToggleQuery(!schedulingQueryEnabled)}
                disabled={!schedulingQueryAllowed}
                title={!schedulingQueryAllowed ? 'Recurso não disponível no seu plano de IA' : undefined}
                className={`relative w-11 h-6 rounded-full transition-colors shrink-0 ${!schedulingQueryAllowed ? 'bg-slate-200 dark:bg-slate-700 cursor-not-allowed opacity-60' : schedulingQueryEnabled ? 'bg-indigo-600' : 'bg-slate-300 dark:bg-slate-600'}`}>
            ...
            {!schedulingQueryAllowed && (<div className="mt-3 px-3 py-2 bg-amber-50 dark:bg-amber-900/10 rounded-lg border border-amber-100 dark:border-amber-900/20">
              <p className="text-xs text-amber-700 dark:text-amber-300">
                    Seu plano de IA atual não inclui consulta de disponibilidade. Faça upgrade do plano de IA para liberar este recurso.
              </p>
            </div>)}
            {schedulingQueryAllowed && schedulingQueryEnabled && (<div className="mt-3 px-3 py-2 bg-blue-50 dark:bg-blue-900/10 rounded-lg border border-blue-100 dark:border-blue-900/20">
        ...
        <div className="flex items-start gap-4 p-4 bg-slate-50 dark:bg-slate-700/30 rounded-xl border border-slate-100 dark:border-slate-700">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between gap-3">
              <h4 className="text-sm font-semibold text-slate-700 dark:text-slate-200 flex items-center gap-1.5">
                Criar Agendamentos
                {!schedulingBookingAllowed && <Lock size={12} className="text-amber-500"/>}
              </h4>
              <button
                onClick={() => schedulingBookingAllowed && onToggleBooking(!schedulingBookingEnabled)}
                disabled={!schedulingBookingAllowed}
                title={!schedulingBookingAllowed ? 'Recurso não disponível no seu plano de IA' : undefined}
                className={`relative w-11 h-6 rounded-full transition-colors shrink-0 ${!schedulingBookingAllowed ? 'bg-slate-200 dark:bg-slate-700 cursor-not-allowed opacity-60' : schedulingBookingEnabled ? 'bg-indigo-600' : 'bg-slate-300 dark:bg-slate-600'}`}>
            ...
            {!schedulingBookingAllowed && (<div className="mt-3 px-3 py-2 bg-amber-50 dark:bg-amber-900/10 rounded-lg border border-amber-100 dark:border-amber-900/20">
              <p className="text-xs text-amber-700 dark:text-amber-300">
                    Seu plano de IA atual não inclui criação de agendamentos. Faça upgrade do plano de IA para liberar este recurso.
              </p>
            </div>)}
            {schedulingBookingAllowed && schedulingBookingEnabled && (<div className="mt-3 px-3 py-2 bg-green-50 dark:bg-green-900/10 rounded-lg border border-green-100 dark:border-green-900/20">
            ...
            {schedulingBookingAllowed && schedulingBookingEnabled && !schedulingQueryEnabled && (<div className="mt-2 px-3 py-2 bg-amber-50 dark:bg-amber-900/10 rounded-lg border border-amber-100 dark:border-amber-900/20">
              <p className="text-xs text-amber-700 dark:text-amber-300">
