...
export default function EditCampaignModal({ isOpen, campaign, onClose, onSuccess }: EditCampaignModalProps) {
  ...
  useEffect(() => {
    if (isOpen && campaign) {
      ...
      setDocumentFileName((campaign.messageMeta as { documentBase64?: string; documentName?: string } | undefined)?.documentBase64
        ? ((campaign.messageMeta as { documentName?: string })?.documentName || 'Documento existente')
        : '');
    }
            ...
            <div className="space-y-4">
              ...
              {(formData.message.trim() || (formData.messageMeta as { imageBase64?: string } | undefined)?.imageBase64) && (
                <div>
                  ...
                  {(formData.messageMeta as { documentBase64?: string } | undefined)?.documentBase64 && (
                    <div className="mt-2 text-xs text-indigo-600 dark:text-indigo-400 flex items-center gap-1.5">
                      <FileText size={12} />
