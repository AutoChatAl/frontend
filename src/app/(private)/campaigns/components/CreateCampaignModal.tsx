  ...
  if (dispatchResult) {
    return (<Modal isOpen={isOpen} onClose={handleClose} title="Campanha Disparada" size="sm">
      <div className="flex flex-col items-center gap-6 py-2">
        ...
        {dispatchResult.jobsCreated > 0 && (<p className="text-xs text-slate-400 text-center">
          {dispatchResult.jobsCreated} job{dispatchResult.jobsCreated !== 1 ? 's' : ''} gerado
          {dispatchResult.jobsCreated !== 1 ? 's' : ''} no total
        </p>)}

        <button onClick={handleClose} className="w-full px-4 py-2.5 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 transition-colors font-medium text-sm">
            Fechar
        </button>
      </div>
    </Modal>);
  }
  return (<Modal isOpen={isOpen} onClose={handleClose} title="Nova Campanha" size="lg">
    {loadingData ? (<div className="flex flex-col items-center justify-center py-16 gap-3">
      ...
      <section>
        ...
        {isOfficialCampaign ? (<div className="space-y-4">
          ...
          {(formData.message.trim() || formData.messageMeta?.imageBase64) && (<div>
            ...
            {formData.messageMeta?.documentBase64 && (<div className="mt-2 text-xs text-indigo-600 dark:text-indigo-400 flex items-center gap-1.5">
              <FileText size={12}/>
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 pt-4 border-t border-slate-200 dark:border-slate-700">
        ...
        <button type="button" onClick={handleCreate} disabled={loading} className="flex items-center justify-center gap-2 px-5 py-2.5 text-sm font-medium border-2 border-indigo-500 text-indigo-600 dark:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-950/50 rounded-xl transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
          {loadingType === 'create' ? (<Loader2 size={16} className="animate-spin"/>) : (<Save size={16}/>)}
          {loadingType === 'create' ? 'Salvando...' : 'Salvar Campanha'}
        </button>

        <button type="button" onClick={handleCreateAndDispatch} disabled={loading} className="flex items-center justify-center gap-2 px-5 py-2.5 text-sm font-medium bg-indigo-600 text-white hover:bg-indigo-700 rounded-xl transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-sm shadow-indigo-500/25">
          {loadingType === 'dispatch' ? (<Loader2 size={16} className="animate-spin"/>) : (<Send size={16}/>)}
