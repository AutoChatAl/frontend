'use client';
import { AlertCircle, ExternalLink, Image as ImageIcon, Instagram, MessageSquare, Mic, Pencil, Plus, Send, Trash2, Type } from 'lucide-react';
...
const REPLY_TYPE_LABELS: Record<string, {
    label: string;
    icon: typeof Type;
}> = {
...
export default function CommentAutomationsPage() {
  ...
  const fetchRules = useCallback(async () => {
    try {
      ...
      const normalized = data.map((r: CommentAutomation & {
                _id?: string;
            }) => ({
        ...r,
        id: r.id || r._id || '',
  ...
  return (<div className="space-y-4 sm:space-y-6 animate-in fade-in duration-500">
    ...
    {rules.length === 0 ? (<EmptyState icon={<MessageSquare size={22}/>} title="Nenhuma automação de comentário" description="Crie automações para responder comentários e enviar DMs quando alguém comentar nas suas publicações do Instagram." action={{
      ...
      {rules.map((rule) => (<div key={rule.id} className={`bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-4 sm:p-5 transition-all ${!rule.enabled ? 'opacity-60' : ''}`}>
          ...
          <div className="flex items-start gap-3 sm:gap-4 flex-1 min-w-0">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex flex-wrap items-center gap-1.5 sm:gap-2 mb-2">
                ...
                {rule.triggerOnAnyComment ? (<Badge type="warning" text="Qualquer comentário" pill/>) : (<Badge type="neutral" text={MATCH_MODE_LABELS[rule.matchMode] || rule.matchMode} pill/>)}
                {rule.commentReplyEnabled && (<span className="inline-flex items-center gap-1 text-[10px] font-medium text-pink-600 dark:text-pink-400 bg-pink-50 dark:bg-pink-950/30 px-2 py-0.5 rounded-full">
                ...
                {rule.dmReplyType && rule.dmReplyType !== 'TEXT' && (() => {
                  ...
                  if (!rt)
                    return null;
                  const Icon = rt.icon;
                  return (<span className="inline-flex items-center gap-1 text-[10px] font-medium text-violet-600 dark:text-violet-400 bg-violet-50 dark:bg-violet-950/30 px-2 py-0.5 rounded-full">
                    <Icon size={10}/> {rt.label}
                  </span>);
                })()}
                {rule.oncePerUser && (<span className="text-[10px] font-medium text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-700 px-2 py-0.5 rounded-full">
                          1x por pessoa
                </span>)}
              </div>
              ...
              <div className="space-y-2">
                ...
                {!rule.triggerOnAnyComment && rule.keyword && (<div>
                ...
                {rule.commentReplyEnabled && rule.commentReplyMessage && (<div>
                ...
                <div>
                  ...
                  <div className="text-sm text-slate-700 dark:text-slate-300 bg-indigo-50 dark:bg-indigo-950/30 px-3 py-1.5 rounded-lg border border-indigo-100 dark:border-indigo-900/50 space-y-1">
                    {rule.dmMessage && (<p className="line-clamp-2 whitespace-pre-wrap">{rule.dmMessage}</p>)}
                    {rule.dmAudioBase64 && (<div className="flex items-center gap-1.5 text-indigo-600 dark:text-indigo-400">
                      <Mic size={12}/>
                      <span className="text-xs font-medium">Mensagem de áudio</span>
                    </div>)}
                    {rule.dmImageBase64 && (<div className="flex items-center gap-1.5 text-indigo-600 dark:text-indigo-400">
                      <ImageIcon size={12}/>
                      <span className="text-xs font-medium">Imagem anexada</span>
                    </div>)}
                    {rule.dmLinkUrl && (<div className="pt-0.5">
    ...
    <CreateCommentAutomationModal isOpen={isCreateOpen} onClose={() => setIsCreateOpen(false)} onSuccess={() => {
      fetchRules();
      addToast('success', 'Automação de comentário criada com sucesso!');
    }}/>

    {editTarget && (<EditCommentAutomationModal isOpen={!!editTarget} onClose={() => setEditTarget(null)} onSuccess={() => {
      fetchRules();
      addToast('success', 'Automação atualizada com sucesso!');
    }} automation={editTarget}/>)}

    {deleteTarget && (<ConfirmDeleteModal isOpen={!!deleteTarget} onClose={() => setDeleteTarget(null)} onConfirm={handleDelete} loading={deleting} title="Excluir Automação" message={`Tem certeza que deseja excluir a automação ${deleteTarget.keyword ? `para "${deleteTarget.keyword}"` : 'de qualquer comentário'}?`}/>)}
