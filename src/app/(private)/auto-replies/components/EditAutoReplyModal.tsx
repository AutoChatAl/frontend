...
export default function EditAutoReplyModal({ isOpen, onClose, onSuccess, autoReply }: EditAutoReplyModalProps) {
  ...
  const buildFormData = useCallback((): UpdateAutoReplyInput => {
    ...
    if (autoReply.replyAudioBase64)
      data.replyAudioBase64 = autoReply.replyAudioBase64;
    if (autoReply.replyAudioMimeType)
      data.replyAudioMimeType = autoReply.replyAudioMimeType;
    if (autoReply.replyImageBase64)
      data.replyImageBase64 = autoReply.replyImageBase64;
    if (autoReply.replyImageMimeType)
      data.replyImageMimeType = autoReply.replyImageMimeType;
    if (autoReply.replyDocumentBase64)
      data.replyDocumentBase64 = autoReply.replyDocumentBase64;
    if (autoReply.replyDocumentMimeType)
      data.replyDocumentMimeType = autoReply.replyDocumentMimeType;
    if (autoReply.replyDocumentName)
      data.replyDocumentName = autoReply.replyDocumentName;
    if (autoReply.replyLinkUrl)
      data.replyLinkUrl = autoReply.replyLinkUrl;
    if (autoReply.replyLinkLabel)
      data.replyLinkLabel = autoReply.replyLinkLabel;
    if (autoReply.replyLinkDescription)
      data.replyLinkDescription = autoReply.replyLinkDescription;
    return data;
  }, [autoReply]);
  ...
  return (<Modal isOpen={isOpen} onClose={onClose} title="Editar Auto-Resposta" size="md">
    <div className="space-y-5">
      ...
      {formData.keyword && (formData.replyMessage || formData.replyAudioBase64 || formData.replyImageBase64) && (<div className="bg-slate-50 dark:bg-slate-900/50 rounded-xl p-4 border border-slate-200 dark:border-slate-700">
      ...
      <ModalActions onCancel={onClose} onConfirm={handleSubmit} confirmLabel="Salvar Alterações" confirmIcon={<Reply size={16}/>} loading={loading} loadingText="Salvando..."/>
    </div>
  </Modal>);
}
