...
export default function EditContactModal({ isOpen, contact, onClose, onSuccess }: EditContactModalProps) {
  ...
  const identifier = identities.length > 0
    ...
    : '';
  return (<Modal isOpen={isOpen} onClose={handleClose} title="Editar Contato" size="md">
    {loadingTags ? (<div className="flex flex-col items-center justify-center py-12 gap-3">
      ...
      <section>
        <div className="flex items-center justify-between mb-3">
          ...
          {selectedTagId && (<span className="text-xs bg-indigo-100 dark:bg-indigo-900/50 text-indigo-600 dark:text-indigo-400 px-2 py-0.5 rounded-full font-medium">
                  1 selecionada
          </span>)}
        </div>

        {allTags.length === 0 && !showNewTagInput ? (<div className="flex flex-col items-center justify-center p-6 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-xl gap-2">
          <Tag size={20} className="text-slate-300 dark:text-slate-600"/>
          <p className="text-sm text-slate-500 dark:text-slate-400 font-medium">
                  Nenhuma tag criada ainda
          </p>
          <button type="button" onClick={() => setShowNewTagInput(true)} className="text-sm text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 font-medium flex items-center gap-1 mt-1">
            <Plus size={14}/>
                  Criar primeira tag
          </button>
        </div>) : (<>
          ...
          {!showNewTagInput ? (<button type="button" onClick={() => setShowNewTagInput(true)} className="flex items-center gap-1.5 text-sm text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300 font-medium transition-colors">
            <Plus size={14}/>
                    Nova tag
