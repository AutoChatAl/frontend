...
export default function SyncContactsModal({ isOpen, onClose, onSuccess, whatsappChannels }: SyncContactsModalProps) {
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      addToast('error', err instanceof Error
        ? err.message
  ...
  return (<Modal isOpen={isOpen} onClose={handleClose} title="Sincronizar Contatos" size="sm">
    <div className="space-y-5">
      ...
      <div>
        ...
        {connectedWA.length === 0 ? (<div className="flex items-start gap-3 p-4 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-xl">
          ...
          {connectedWA.map((channel) => {
            ...
            return (<label key={channel.id} className={`flex items-center gap-3 p-3.5 rounded-xl border-2 cursor-pointer transition-all select-none ${isSelected
              ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-950/40'
              : 'border-slate-200 dark:border-slate-700 hover:border-slate-300 dark:hover:border-slate-600'}`}>
              <input type="radio" name="sync-channel" value={channel.id} checked={isSelected} onChange={() => setSelectedChannelId(channel.id)} className="sr-only"/>
              ...
              <div className="flex-1 min-w-0">
                ...
                {channel.number && (<p className="text-xs text-slate-400 dark:text-slate-500 font-mono">
                          +{channel.number}
                </p>)}
              </div>
              ...
              <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center shrink-0 transition-all ${isSelected
      ...
      <ModalActions onCancel={handleClose} onConfirm={handleSync} confirmLabel="Sincronizar" confirmIcon={<RefreshCw size={16}/>} loading={syncing} loadingText="Sincronizando..." disabled={!selectedChannelId}/>
    </div>
  </Modal>);
}
