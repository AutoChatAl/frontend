...
export function SubscriptionProvider({ children }: {
  ...
  const refreshAfterPurchase = useCallback(async (opts?: {
        expectPlanId?: string;
        expectActive?: boolean;
        tries?: number;
    }) => {
  ...
  const value = useMemo(() => ({ status, usage, loading, refresh, refreshAfterPurchase, ...derived }), [status, usage, loading, refresh, refreshAfterPurchase, derived]);
...
export function usePlanLimitCheck(resource: keyof UsageSummary) {
  ...
  if (!entry)
    return { used: 0, limit: 0, remaining: 0, isAtLimit: false, percentage: 0 };
