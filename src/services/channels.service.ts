...
class ChannelsService {
  public async getWhatsAppInstances(): Promise<WhatsAppInstance[]> {
    ...
    if (!response.success || !response.data) throwApiError(response, 'Falha ao buscar instancias do WhatsApp. Tente novamente.');
  ...
  public async createWhatsAppInstance(data: {
    ...
    if (!response.success || !response.data) throwApiError(response, 'Nao foi possivel criar a instancia do WhatsApp. Tente novamente.');
  ...
  public async connectWhatsAppInstance(
    ...
    if (!response.success || !response.data) throwApiError(response, 'Nao foi possivel conectar a instancia do WhatsApp. Tente novamente.');
  ...
  public async getWhatsAppQRCode(channelId: string): Promise<WhatsAppQRCodeRawResponse> {
    ...
    if (!response.success || !response.data) throwApiError(response, 'Nao foi possivel obter o QR code. Tente novamente.');
  ...
  public async getWhatsAppStatus(channelId: string): Promise<WhatsAppStatusResponse> {
    ...
    if (!response.success || !response.data) throwApiError(response, 'Nao foi possivel obter o status do WhatsApp. Tente novamente.');
  ...
  public async deleteWhatsAppInstance(id: string): Promise<void> {
    ...
    if (!response.success) throwApiError(response, 'Nao foi possivel deletar a instancia do WhatsApp. Tente novamente.');
  }
  ...
  public async getInstagramAccounts(): Promise<InstagramAccount[]> {
    ...
    if (!response.success || !response.data) throwApiError(response, 'Nao foi possivel buscar contas do Instagram. Tente novamente.');
  ...
  public async getInstagramOAuthUrl(): Promise<{ url: string }> {
    ...
    if (!response.success || !response.data) throwApiError(response, 'Nao foi possivel obter a URL de OAuth do Instagram. Tente novamente.');
  ...
  public async connectInstagramAccount(code: string): Promise<InstagramAccount> {
    ...
    if (!response.success || !response.data) throwApiError(response, 'Nao foi possivel conectar a conta do Instagram. Tente novamente.');
    return response.data as InstagramAccount;
  }

  public async deleteInstagramAccount(id: string): Promise<void> {
    const response = await apiClient.delete(`/channels/instagram/${id}`);
    if (!response.success) throwApiError(response, 'Nao foi possivel deletar a conta do Instagram. Tente novamente.');
  }
}
