...
function normalizeId<T extends {
    id?: string;
    _id?: string;
}>(obj: T): T {
  if (!obj.id && obj._id) {
...
export class CampaignService {
  ...
  public async createCampaign(input: CreateCampaignInput): Promise<Campaign> {
    const response = await apiClient.post<BackendResponse<Campaign & {
            _id?: string;
        }>>('/campaigns', input);
    const data = extractData<Campaign & {
            _id?: string;
        }>(response, 'Falha ao criar campanha. Tente novamente.');
  ...
  public async runCampaign(campaignId: string, paceMs?: number): Promise<CampaignRun> {
    ...
    const response = await apiClient.post<BackendResponse<CampaignRun>>(`/campaigns/${campaignId}/run${queryString}`);
  ...
  public async deleteCampaign(campaignId: string): Promise<void> {
    ...
    if (!response.success)
      throw new Error('Falha ao excluir campanha. Tente novamente.');
  }
  public async processJobs(opts?: {
        limit?: number;
        runId?: string;
    }): Promise<{
        sent: number;
