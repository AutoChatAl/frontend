...
class AutoReplyService {
  public async list(): Promise<AutoReply[]> {
    ...
    if (!response.success || !response.data) {
      throwApiError(response, 'Não foi possível buscar respostas automáticas. Tente novamente.');
    }
  ...
  public async getById(id: string): Promise<AutoReply> {
    ...
    if (!response.success || !response.data) {
      throwApiError(response, 'Não foi possível buscar a resposta automática. Tente novamente.');
    }
  ...
  public async create(input: CreateAutoReplyInput): Promise<AutoReply> {
    ...
    if (!response.success || !response.data) {
      throwApiError(response, 'Não foi possível criar a resposta automática. Tente novamente.');
    }
    return response.data as AutoReply;
  }
  public async update(id: string, input: UpdateAutoReplyInput): Promise<AutoReply> {
    ...
    if (!response.success || !response.data) {
      throwApiError(response, 'Não foi possível atualizar a resposta automática. Tente novamente.');
    }
    return response.data as AutoReply;
  }
  public async delete(id: string): Promise<void> {
    ...
    if (!response.success) {
      throwApiError(response, 'Não foi possível deletar a resposta automática. Tente novamente.');
    }
  }
