...
import type { SupportChatConversation, SupportChatListFilters, SupportChatMessage, SupportChatPublicConversation, SupportChatSendMessageInput, SupportChatSummary } from '@/types/SupportChat';
...
function makeApiError(defaultMessage: string, statusCode: number, payload?: ApiErrorPayload): Error {
  const err = new Error(getSupportChatErrorMessage(payload?.reason || payload?.code, payload?.message || defaultMessage) || defaultMessage) as Error & {
        code?: string;
        statusCode?: number;
    };
...
class SupportChatService {
  public getPublicWorkspaceId(): string | null {
    return (process.env.NEXT_PUBLIC_SUPPORT_WORKSPACE_ID ||
  ...
  public async createPublicConversation(input: {
    ...
    const response = await apiClient.post<{
            data: SupportChatPublicConversation;
        }>('/support-chat/public/conversations', input);
  ...
  public async sendPublicMessage(conversationId: string, visitorToken: string, input: SupportChatSendMessageInput): Promise<{
    ...
    const response = await apiClient.post<{
        ...
        }>(`/support-chat/public/conversations/${conversationId}/messages?visitorToken=${encodeURIComponent(visitorToken)}`, input);
    if (!response.success || !response.data) {
  ...
  public async listAdminConversations(filters: SupportChatListFilters = {}): Promise<SupportChatConversation[]> {
    ...
    const response = await apiClient.get<{
            data: SupportChatConversation[];
        }>(`/support-chat${suffix}`);
    if (!response.success || !response.data) {
      if ((response.data as {
                reason?: string;
            } | undefined)?.reason === 'MIXED_CONTENT_BLOCKED') {
  ...
  public async sendAdminMessage(conversationId: string, input: SupportChatSendMessageInput): Promise<{
        message: SupportChatMessage;
        conversation: SupportChatConversation;
    }> {
    const response = await apiClient.post<{
        ...
        }>(`/support-chat/${conversationId}/messages`, input);
    if (!response.success || !response.data) {
  ...
  public async closeConversation(conversationId: string): Promise<SupportChatConversation> {
    const response = await apiClient.patch<{
            data: SupportChatConversation;
        }>(`/support-chat/${conversationId}`, { action: 'close' });
    if (!response.success || !response.data) {
